...
for j in [2,3]:
    data = np.ones((8, 5)) *np.nan
    newfname = f'a22{j}_annu_pandas_table_ppspcm2parcmin2_sep.csv'
    area = cal_area(rlst[f'a22{j}'], oplst[f'a22{j}'])
    print(area)
    for i in range(5):
        ctsfiles = glob(f'a22{j}_annu_{i+1}_pandas_ds9_new_*_purects.log')
        for ctsfile in ctsfiles:
            n = int(ctsfile.split('.')[0].split('_')[-2])-1
            expfile = f'a22{j}_annu_{i+1}_pandas_ds9_new_{n+1}_scm2.log'
            f = open(ctsfile)
            print(n,i,ctsfile)
            ctslst = f.readlines()
            f.close()
            ...
            explst = f.readlines()
            f.close()
            if 'Error' in ctslst[1]:
                continue
            else: 
                cts = float(ctslst[3].split('=')[-1])
                exp = float(explst[3].split('=')[-1])
                data[n][i] = (cts)/exp

    data[5] = np.nanmean(data[:5], axis=0)
    data[6] = np.nanstd(data[:5], axis=0)
    data[7] = data[6]/data[5]*100
    df = pd.DataFrame(data, index=[f'panda{i+1}' for i in range(5)] + ['mean', 'std','std/mean(%)'],columns=[f'annu{i+1}' for i in range(5)])
    df.to_csv(newfname)
    print(df)
